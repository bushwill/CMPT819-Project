...
def detect_board_and_rings(edges, config):
    """Detect outer board circle and inner rings using pattern matching. Returns None if not a valid board."""
    h, w = edges.shape[:2]
    ...
    # OPTIMIZATION: Do ALL Hough transforms upfront (outer + inner in one go)
    ...
    # Get outer board candidates (large radii only)
    outer_candidates = [c for c in all_circles if c['radius'] >= min_radius]
    outer_candidates.sort(key=lambda x: x['radius'], reverse=True)
    
    # Try pattern matching for each outer candidate
    for board_candidate in outer_candidates:
        board_center = board_candidate['center']
        board_radius = board_candidate['radius']
        
        detected_rings = {'outer': board_radius}
        max_offset = board_radius * ring_search_cfg['max_center_offset']
        
        # Match expected ring ratios from pre-computed circles
        ...
        # Check if we found all 5 rings (complete pattern match)
        if len(detected_rings) == 5:
            # Refine the board radius using the inner rings as reference
            if 'ring_5' in detected_rings:
                refined_outer = int(detected_rings['ring_5'] / ring_ratios['ring_5'])
                if abs(refined_outer - board_radius) < board_radius * 0.15:
                    board_radius = refined_outer
                    detected_rings['outer'] = board_radius
            
            print(f"Board found: center={board_center}, radius={board_radius}")
            return {
                'center': board_center,
                'radius': board_radius,
